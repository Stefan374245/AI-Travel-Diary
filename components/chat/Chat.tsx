...
const Chat: React.FC<ChatProps> = ({ savedEntries }) => {
  ...
  const handleSaveFlashcard = async (flashcard: Flashcard) => {
    ...
    const savedCard = await saveFlashcard(flashcard, undefined, entryData);
    ...
    <Card variant="default" padding="none" className="flex flex-col h-[70vh] overflow-hidden">
      <div className="p-6 border-b border-neutral-200">
        <Stack spacing="sm">
          ...
          <div ref={optionsDropdownRef} className="relative mt-2">
            <button
              type="button"
              onClick={() => setIsOptionsOpen(!isOptionsOpen)}
              className="w-full flex items-center justify-between gap-3 text-sm border-2 border-primary-300 bg-primary-50 rounded-xl shadow-sm px-4 py-3 hover:bg-primary-100 hover:border-primary-400 focus:outline-none focus:ring-2 focus:ring-primary-500 transition-colors"
            >
              <div className="flex items-center gap-2">
                <svg className="w-5 h-5 text-primary-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6V4m0 2a2 2 0 100 4m0-4a2 2 0 110 4m-6 8a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4m6 6v10m6-2a2 2 0 100-4m0 4a2 2 0 110-4m0 4v2m0-6V4" />
                </svg>
                <Text variant="body" className="font-semibold text-primary-900">Chat-Optionen</Text>
              </div>
              <svg 
